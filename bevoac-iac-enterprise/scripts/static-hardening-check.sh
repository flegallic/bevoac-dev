...
api_legacy_admin_reader_block="$(grep -A8 'resource "azurerm_role_assignment" "api_legacy_admin_secret_reader"' workload-security-phase1.tf || true)"
[[ -n "$api_legacy_admin_reader_block" ]] || fail "Legacy API admin-secret reader transition is missing."
grep -q 'count .*var.retain_legacy_api_admin_secret_reader ? 1 : 0' <<<"$api_legacy_admin_reader_block" || fail "Legacy API admin-secret reader must be conditionally removable."
worker_servicebus_reader_block="$(grep -A8 'resource "azurerm_role_assignment" "worker_servicebus_secret_reader"' workload-security-phase1.tf || true)"
[[ -n "$worker_servicebus_reader_block" ]] || fail "Legacy worker Service Bus secret reader transition is missing."
grep -q 'count .*var.retain_legacy_servicebus_connection_secret ? 1 : 0' <<<"$worker_servicebus_reader_block" || fail "Legacy worker Service Bus secret reader must follow the legacy secret lifecycle."
grep -q 'to .*azurerm_role_assignment.api_legacy_admin_secret_reader\[0\]' legacy-rbac-transition-v6-1-3.tf || fail "Legacy API admin-secret reader state move is missing."
grep -q 'to .*azurerm_role_assignment.worker_servicebus_secret_reader\[0\]' legacy-rbac-transition-v6-1-3.tf || fail "Legacy worker Service Bus secret reader state move is missing."
grep -q 'retain_legacy_api_admin_secret_reader *= *true' release/v6.1.3-workload-migration.tfvars.example || fail "Workload profile must retain the legacy API admin-secret reader."
grep -q 'retain_legacy_api_admin_secret_reader *= *false' release/v6.1.3-security-finalize.tfvars.example || fail "Security profile must remove the legacy API admin-secret reader."
grep -q "'retain_legacy_api_admin_secret_reader': True" ../scripts/release/deploy_v6_1_3.sh || fail "Workload runner must retain the legacy API admin-secret reader."
grep -q "'retain_legacy_api_admin_secret_reader': False" ../scripts/release/deploy_v6_1_3.sh || fail "Security runner must remove the legacy API admin-secret reader."
grep -q 'api_legacy_admin_secret_reader|worker_servicebus_secret_reader' ../scripts/release/deploy_v6_1_3.sh || fail "Security plan deletion allowlist is missing legacy secret-scoped readers."
grep -q 'public API still reads admin-api-secret' ../scripts/release/deploy_v6_1_3.sh || fail "Final smoke must verify legacy API admin-secret access removal."
grep -q 'worker still reads the legacy Service Bus connection secret' ../scripts/release/deploy_v6_1_3.sh || fail "Final smoke must verify legacy worker Service Bus secret access removal."
pass "Service Bus and legacy secret-scoped RBAC use a staged rollback-safe transition."
