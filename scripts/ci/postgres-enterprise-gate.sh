...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_api') THEN CREATE ROLE bevoac_api LOGIN PASSWORD 'ci_api_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_worker') THEN CREATE ROLE bevoac_worker LOGIN PASSWORD 'ci_worker_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_outbox') THEN CREATE ROLE bevoac_outbox LOGIN PASSWORD 'ci_outbox_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_retention') THEN CREATE ROLE bevoac_retention LOGIN PASSWORD 'ci_retention_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_admin_api') THEN CREATE ROLE bevoac_admin_api LOGIN PASSWORD 'ci_admin_api_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='bevoac_operator') THEN CREATE ROLE bevoac_operator LOGIN PASSWORD 'ci_operator_password' NOSUPERUSER NOCREATEROLE NOCREATEDB NOINHERIT NOREPLICATION NOBYPASSRLS; END IF;
END $$;
CREATE DATABASE bevoac_ci OWNER bevoacadmin;
REVOKE ALL ON DATABASE bevoac_ci FROM PUBLIC;
GRANT CONNECT ON DATABASE bevoac_ci TO bevoacadmin, bevoac_api, bevoac_worker, bevoac_outbox, bevoac_retention, bevoac_admin_api, bevoac_operator;
SQL
...
BEGIN
  ...
  SELECT count(*) INTO membership_count FROM pg_auth_members m JOIN pg_roles r ON r.oid=m.member WHERE r.rolname IN ('bevoac_api','bevoac_worker','bevoac_outbox','bevoac_retention','bevoac_admin_api','bevoac_operator');
  ...
  IF migration_count <> 8 OR enabled_count <> 15 OR forced_count <> 15 OR policy_count <> 29 OR hardened_roles <> 6 OR membership_count <> 0 OR grant_count <> 58 THEN
    RAISE EXCEPTION 'Enterprise DB gate failed: migrations %, enabled %, forced %, policies %, roles %, memberships %, grants %', migration_count, enabled_count, forced_count, policy_count, hardened_roles, membership_count, grant_count;
  END IF;
END $$;
SQL
