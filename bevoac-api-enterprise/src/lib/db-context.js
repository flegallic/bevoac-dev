...
async function withDatabaseSession(pool, fn) {
  ...
  try {
    ...
    // Application errors do not poison a PostgreSQL connection. Destroy only
    // clients that failed at the protocol/network level.
    releaseError = isConnectionFailure(error) ? error : null;
    throw error;
  } finally {
...
async function withTenantSession(pool, tenantId, fn) {
  // Tenant operations are intentionally transaction-scoped. PostgreSQL
  // clears SET LOCAL state on COMMIT/ROLLBACK, so a tenant identifier cannot
  // leak into the next borrower of a pooled connection.
