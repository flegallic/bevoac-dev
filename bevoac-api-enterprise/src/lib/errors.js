...
class ConflictError extends AppError { constructor(message = 'Conflict', details = null, code = 'CONFLICT') { super(message, { code, statusCode: 409, details }); } }
class IdempotencyConflictError extends ConflictError { constructor(message = 'Idempotency key was already used for a different request.', details = null) { super(message, details, 'IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST'); } }
...
module.exports = { AppError, UnauthorizedError, ForbiddenError, NotFoundError, ConflictError, IdempotencyConflictError, ValidationError };
